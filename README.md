...
<div align="center">
  ...
  <p>Realice un  video de como funciona en vivo, puedes darle clik a la imagen </p>
</div>

Bot de **scalping multi–par**  que trabajan con una combiacion de estregias de Market Bias + Twin Range  y PSAR en 30m  La razon princpial por lo que me enfoco en esta combinacion es por que arroja resulatdos no perfectos pero si cosideerables en el  backetste  de TradingView es por que quise probar de forma mas objetvia y al combianar los idcadores no queria unicamente poner todos los filtors y esperar que en algun momento se alinearan, bueno si lo intente pero resulto muy poco inefciente super horcado, por lo que se hice fue genersr una ponderacion por indicador y en base a eso se ecide con que pares se va a trabajr y en que dioreccion se abre   construido en Python, que combina para tomar deciones:
...
El proyecto  esta creado poara probar la estragia en entorno mas cercano a lo real  y ver comon es que reacciona Usando (indicadores, timeframes, riesgo, fees)  para saber si encontraba algo de consistencia para tener una nocion de su desempeño con datos reales sin ariesgar capital. 

---

##  Visión general del proyect
...
El objetivo no es tener una recveta magica , sino  crear y aprender con todo el sistema y como es que trabaja



## El enfoque es ver de forma clara su resulatado 

Con una  estrategia basada en PSAR 30m, filtros de Market Bias + Twin Range Filter en 5m y un sistema de scoring adaptativo (ADX, ATR, EMA200 4H) compensar comisiones y volatilidad en Binance Futures usando posiciones pequeñas y reglas de riesgo sistemáticas?
...
##  Núcleo de la lógica: score y priority 
...
### Macro vs Micro con pesos dinámicos (ADX)
...
###  Sesgo adaptativo con EMA200 4H (bias pro–trend)
...
### ATR: de “veto” a escalado de tamaño
...
### Score = confianza = tamaño (RRR constante)
...
## Indicadores y lógica por timeframe

###  4H: EMA 200 (bias macro)
...
###  5m: Market Bias y Twin Range Filter
...
##  Logging y análisis de resultados
